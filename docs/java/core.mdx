...
1. Promotion rule and <Term>Casting</Term> will occur during assignment.
...
1. Casting is performed by placing the data type, enclosed in parentheses, to the left of the value you want to cast.
    ...
    // This will error
    short b = 40000; // Compilation error
    short c = (short) 40000;
...
1. Compiler <Warn>does not</Warn> **require casting when working with literals** that fit into data types.
    ...
    double h = 2 * 5.0;
    ```
1. When working with **literals / value**, the compiler has enough information for the programmer's intent. Casting is not required.
1. When working with **variables**, there is ambiguity about how to proceed, and the compilers will throw error. Casting is required.
...
1. We can include && statement with pattern variable that declared on the same line to filter data out.
    ...
    ```
1. Pattern variable must be a strict subtype. It <Warn>cannot</Warn> **even** be the **same** type.
    ```java
    Integer a = 1000;
    // This will error
