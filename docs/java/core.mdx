...
### Environment
1. `JDK`: <Term>Java Development Kit</Term>
    - `javac`: Convert .java (source file) into .class (bytecode)
    - `java`: Execute
    - `jar`: Package
    - `javadoc`: Document
1. `JRE`: <Term>Java Runtime Environment</Term>
    - Subset of JDK
    - <Warn>Cannot</Warn> compile
1. `IDE`: <Term>Integrated Development Environment</Term>

### Class Structure
#### Field and Method
1. <Term>Method</Term>: a.k.a. *function* or *procedure* in other languages
1. <Term>Field</Term>: a.k.a. *variable*
1. Method and Field are <Term>Members</Term> of a <Term>Class</Term>.
1. `void`
    - Special return type
    - No value is returned
1. <Term>Method Signature</Term> is the combination of **method name** and **parameter types**.
#### Classes and Source Files
1. **Class** is a <Term>Top level type</Term>.
1. You can put two or more classes in a file.
1. Within the file, at most one of the top level type is allowed to be public.
1. If you have a public top level type, it must match the filename.
#### main() method
1. Method `main()` is for the execution of a program.
    ```java
    ...
    ```
    - `public` is an <Term>Access modifier</Term>.
    - `static` is used as no object needs to be created to run the program.
    - `...` is called `varargs` <Term>Variable argument lists</Term>.
...
1. Shortcut for running single file source code.
    ...
    - This feature is designed for scenario where your program is in one single file, such as unit testing.
### Package and Import
1. <Term>Package</Term> is the logical grouping for classes.
1. Package name is hierarchical, separated by period `.`.
    ...
    ```
1. Directory structure must follow the package name.
1. <Term>Import</Term> statement tells which package to look in for classes.
    ```java
...
1. Below are limitations for import statements.
    - Wildcard can only match class names. It <Warn>cannot</Warn> match directories or subdirectories.
    - There can only be 1 wildcard, and it must be placed at the end.
    - Import statement can only be used to match class names. It <Warn>cannot</Warn> match methods or fields.
1. Naming conflicts
    - Class names don't have to be unique across all of the Java classes.
    - The same class name can be found in multiple places under different packages.
...
1. You can compile with command `javac`.
    ```txt
    javac irushu/chapter1/Test1.java
    javac irushu/chapter1/*.java
    // This will error
    javac *.java // This will not compile any source file under package irushu.chapter1
    ```
...
1. You can specify the classpath and run the program with command `java`.
    ```txt
    ...
    ```
1. You can create jar file with command `jar`.
    ```txt
    jar -cvf test.jar .
...
### Creating objects
1. You can define <Term>Constructor</Term> for the creation of a object.
2. Below is an example of constructor. Note it does <Warn>not</Warn> have return type.
    ...
    ```
1. If you don't provide a constructor, compiler will supply a **do nothing**, **no argument** <Term>Default constructor</Term> for you.
1. You can create object with the keyword `new`.
    ```java
    Test test = new Test();
    ```
1. Caller can read and write instance variables directly. 
    ...
    ```
1. However, you should honour encapsulation and protect instance variables with access modifier.
1. You can define <Term>Instance initializer</Term> as below.
...
1. There are 8 <Term>Primitive types</Term>.
...
1. When a number, decimal, String or character is present in the code, they are called <Term>Literals</Term>.
...
1. Although float values can be declared with an f suffix, they are not printed with an f suffix.
    ...
    ```
1. You can specify a number by **changing base**. By default it is using decimal number system.
...
1. You can put underscores in numbers. 
    ...
    int value = 1_000_000;
    ```
1. You <Warn>cannot</Warn> put underscore at the beginning or end of a literal, or right before or after a decimal point.
    ```java title="irushu.chapter1.Test9"
    // This will error
    double a = _999.00; // Fail to compile
    // This will error
    double b = 999.00_; // Fail to compile
    // This will error
    double c = 999._00; // Fail to compile
    // This will error
    double d = 999_.00; // Fail to compile
    ```
### Reference Type
1. A <Term>Reference type</Term> refers to an object.
...
1. A reference can be assigned to a new object.
### Primitive Type vs Reference Type
1. Primitive types have lowercase type names.
1. Reference types have uppercase type names.
1. Reference types can call methods.
1. Primitive types do not have methods declared on them.
1. If the value of a primitive type is unknown, you may assign null to its wrapper class.
### Wrapper Class
1. Use `valueOf` to convert a String to its wrapper class.
1. Use `parseInt` to convert a String to its primitive class.
    ```java title="irushu.chapter1.Test10"
    Integer a = Integer.valueOf("456"); // Wrapper
    int b = Integer.parseInt("789"); // Primitive
    ```
1. Number classes, Boolean and Character Wrapper classes all have useful helper methods:
    - byteValue(), shortValue(), intValue(), longValue()
    - floatValue(), doubleValue()
    - booleanValue()
    - charValue()
1. Helper classes try their best to convert values but may result in a loss of precision.
### Text Block
...
1. Escape characters
### Declaring variables
1. <Term>Identifier</Term> is the name of a variable, method, class, interface, or package.
1. Identifiers **must** begin with letter, a currency symbol or undercore symbol _ .
1. Currency symbol includes dollar ($), yuan (¥), euro (€).
1. A single underscore is <Warn>not</Warn> allowed.
1. <Warn>Cannot</Warn> use reserve word.
1. <Warn>Cannot</Warn> use literal values like true, false or null.
### Case convention
...
1. You <Warn>cannot</Warn> repeat the same declaration, even the declarations are of the same type.
1. There can only be one declaration, <Warn>no</Warn> other declaration can be made.
### Initializing variables
1. <Term>Local variable</Term> is a variable defined within a constructor, method, or initializer block.
1. `final` modifier is equivalent to declaring constants.
1. `final` modifier can apply to local variable.
1. For `final` variable of primitive type, you <Warn>cannot</Warn> change its value once it is initialized.
1. For `final` variable of reference type, you <Warn>cannot</Warn> change the reference, but you can modify the content the reference points to.
1. Local variables do not have **default value**.
1. Local variables must be initialized before use.
1. For local variables that have never been used, compiler doesn't care if they are not initialized.
...
1. <Term>Instance variable</Term> is a field defined within a specific instance of object.
1. <Term>Class variable</Term> is defined on the class level and shared among all instances of the class.
...
1. Initialization of instance and class variables sticks to below rules.
    ...
    - false for boolean
    - '\u000' for char
1. Java does not support setting default method parameter values. Javascript does.
    ```java title='Java does not support default parameter value'
    // This will error
    public void method(int hello = 3){}
    ```
    ```js title='Javascript supports default parameter value'
    ...
    ```
    To achieve default parameters, Java uses <Term>Overloading</Term>.
    ```java
    public void method(){
        method(3);
    }
    public void method(int hello){
...
1. This feature is called <Term>Local variable type inference</Term>.
...
1. But in java, compiler will determine the specific type for `var`. The type <Warn>cannot</Warn> be changed during runtime.
1. `var` variable <Warn>cannot</Warn> be initialized to null during declaration. Compiler <Warn>cannot</Warn> infer its type.
1. If the underlying type is a reference type, `var` variable can be reassigned with null value after declaration.
1. `var` <Warn>cannot</Warn> be used in multiple variable assignment.
1. For `var`, compiler will look only at the line of declaration. variable **must** be assigned values on the lines where they are defined.
    ...
    void methodA(){
        ...
        var b
            = 5;
        // This will error
        var c; // var variable is not assigned with values on the lines they are declared.
        c = 1;
        // This will error
    ...
    ```
1. `var` <Warn>cannot</Warn> be used on method parameters.
   ```java
    // This will error
...
1. `var` is not a reserved word and allowed to be used as an identifier.
    ...
    ```
### Variable scope
1. A set of braces means a new <Term>Code block</Term>.
...
### Garbage collection
1. For <Term>Garbage collection</Term>, refer to [JVM section](jvm.md).
1. System.gc() is not guaranteed to do anything. JVM can ignore it.

## 2. Operators
1. An <Term>Operator</Term> is a special symbol or operation that can be applied to operands.
1. An <Term>Operand</Term> is the variable / literal / value the operator being applied to.
...
1. Some operators require operand(s) of specific type.
...
1. <Term>Bitwise complement operator ( ~ )</Term> flips all 0s and 1s in a number. 
...
1. You can use the formula `bitwise complement = -1 * number value -1` to calculate the result for bitwise complement operation.
    ...
    ```
1. <Term>Negation operator ( - )</Term> reverses the sign of numeric expression.
1. Negation operator can apply only on numeric expression.
1. <Term>Increment and decrement operators ( ++ -- )</Term> can apply only on numeric variable.
...
1. Division: `a / b`
1. Modulus: `a % b`
1. <Term>Arithmetic operators</Term> can apply on all primitives, <Warn>except</Warn> boolean.
...
1. For **integer** values, **division** results in the **floor value** (value without anything after the decimal point) of the nearest integer that fulfills the operation.
    ...
    ```
### Numeric Promotion
1. Numeric <Term>Promotion rules</Term>
...
### Assignment Operator
1. Assign variable on the left side with the result of the equation on the right side.
1. Both the promotion rule and <Term>Casting</Term> will apply during assignment.
1. Automatically promote from smaller to larger data type.
    ...
    ```
1. Compilation error will occur if you try to promote from larger to smaller data type without casting.
...
1. Casting is performed by placing the data type, enclosed in parentheses, to the left of the value you want to cast.
    ...
    short a = 10; // Literals that fits to the data type does not need casting
    // This will error
    short b = 40000; // 40000 is larget than what the type short can hold. Compilation error occurs.
    short c = (short) 40000;
    System.out.println(c); // Overflow occur, print -25536.
    float d = 10;
    float e = (float) 10.0;
    float f = 10.0f;
    // This will error
...
1. Casting an object only change the reference type of the object, not the object itself.
1. <Term>Overflow</Term> occurs when a number is so large that it will no longer fit within the data type, so the system “wraps around” to the lowest negative value and counts up from there.
    ```java title="irushu.chapter2.Test7a"
    int a = (byte)( Byte.MAX_VALUE + 1 );
    System.out.println(a); // Print -128
    ```
1. <Term>Underflow</Term> is an analogy to overflow when the number is too low to fit in the data type.
...
1. Compiler <Warn>does not</Warn> **require casting when working with literals** that fit into data types.
    ...
    double h = 2 * 5.0;
    ```
1. When working with **literal**, the compiler has enough information for the programmer's intent. Casting is not required.
1. When working with **variable**, there is ambiguity about how to proceed, and the compiler will throw error. Casting is required.
...
1. <Term>Compound assignment</Term> is a glorified form of simple assignment. It performs the following:
    ...
    ```java title="irushu.chapter2.Test10"
        ...
        // Highlight next line
        a = (int) (a * b); // Explicit casting from long to int
        // Highlight next line
        a *= b; // Implicit casting from long to int
    ```
:::tip In my opinion
The promotion rules and casting depicted above are not intuitive, and are therefore error-prone. In the first place, why do we perform operations of mixed data type? Avoid it at all cost. Second, casting from larger data type to smaller one doesn't make sense: Anything leading to a loss of precision, overflow or underflow is highly undesirable. Avoid it at all cost. Unless under extreme circumstances that saving memory space is crucial, always use large data type.
:::
### Equality Operators
...
1. `>`, `>=`, `<`, `<=`, `instanceof`
1. Operator `instanceof` tests whether the target object is a member of a particular class or interface at runtime.
1. If the compiler can determine that a variable <Warn>cannot</Warn> possibly be cast to a specific class, it reports an error.
...
1. instanceof on a null literal / reference will always return false.
    ...
    ```
1. null <Warn>cannot</Warn> be used on the right side of instanceof.
    ...
    ``````
### Logical Operator
1. <Term>Logical AND</Term> `&`: true when both operands are true.
1. <Term>Logical OR</Term> `|`: true when at least one of operands is true.
1. <Term>Logical XOR</Term> `^`: true when operands are different to each other.
### Conditional Operator
1. <Term>Conditional AND</Term> `&&`: true when both operands are true.
1. <Term>Conditional OR</Term> `||`: true when at least one of operands is true.
1. <Term>Short circuit</Term>
...
1. Avoid null pointer exception.
    ...
    ```
### Ternary Operator
1. <Highlight color="grey" fontWeight="700">(boolean expression)</Highlight> ? <Highlight color="grey" fontWeight="700">( expression that returned if boolean is true )</Highlight> : <Highlight color="grey" fontWeight="700">( expression that returned if boolean is false )</Highlight>
...
1. <Term>Control Flow Statement</Term> breaks up the flow of execution by
    - <Term>Decision making</Term> (*if*, *switch*)
    - <Term>Looping</Term> (*while*, *do/while*, *for*, *for-each*)
    - <Term>Branching</Term> (*Nested loop*, *continue*, *break*, *return*)
...
1. Java 16 introduces <Term>Pattern matching</Term> for if statement using `instanceof` operator.
...
1. From the above example, *dog* is the <Term>Pattern variable</Term>. 
...
1. We can include && statement with pattern variable that declared on the same line to filter data out.
    ...
    ```
1. Pattern variable must be a strict **subtype**. **Even** the **same type** is <Warn>not</Warn> allowed.
    ```java
    Integer a = 1000;
    // This will error
    if(a instanceof Integer b){} // Same type is not allowed!
    ```
1. <Term>Flow scoping</Term> means the variable is in scope only when the compiler can definitely determine its type.
...
1. Pattern matching is quite different from other scoping. It is not determined hierarchically by whether the variable is within a pair of braces. It is determined by compiler whether the area is in scope.
...
1. *case* value must be **compile time constant expression**. If the case value <Warn>cannot</Warn> be evaluated until runtime, it will fail the compilation.
    ```java
    static int getValue(){
        return 2;
    }
...
1. Starting with Java 14, <Term>Switch expression</Term> is supported.
    ```java
    byte a = 2;
    byte b = 4;
    byte c = switch (a) {
...
1. Keyword `yield` (exit the switch block) is provided to distinguish it from the keyword `return` (exit the method).
...
1. For nested loops, without a label, the continue statement will end the current iteration of the inner loop.
...
1. Doesn't need to be instantiated with the keyword `new`.
1. Text block can create String.
    ```java
    String hello = """
                    Hello World""";
    ```
1. Implements the interface [`CharSequence`](https://docs.oracle.com/javase/8/docs/api/java/lang/CharSequence.html).
...
1. Illustration of String methods.
    ```java
    ...
    ```
    :::info
    [Difference between strip() and trim()](https://stackoverflow.com/questions/51266582/difference-between-string-trim-and-strip-methods-in-java-11)
    :::
...
1. `equals()` of String is overridden, and checks value equality.
    ...
    ```
1. String literal is pooled in <Term>String Pool</Term>.
    ```java
    var a1 = "Hello World";
    var a2 = "Hello World";
    var a3 = "Hello" + " " + "World";
...
1. `binarySearch`
    ...
    - Found: return index.
    - Not Found:  -(Index of larger and nearest match) -1 .
    ```java
...
1. 3 things: **Date**, **Time** and **Timezone**.
    1. [<Term>LocalDate</Term>](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/LocalDate.html) contains Date only.
    1. [<Term>LocalTime</Term>](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/LocalTime.html) contains Time only.
    1. [<Term>LocalDateTime</Term>](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/LocalDateTime.html) contains Date and Time only.
    1. [<Term>ZonedDateTime</Term>](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/ZonedDateTime.html) contains Date, Time and Timezone.
    ```java
...
1. `plus` and `minus` method
    ...
    minusNanos()
    ```
1. [<Term>Period</Term>](https://docs.oracle.com/javase/8/docs/api/java/time/Period.html) class
    ```java
...
1. Be aware of using invalid period on objects. For example, time has no years, months or days.
    ...
    ```
1. [<Term>Duration</Term>](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/Duration.html) class is for smaller units of time.
...
1. Duration has a generic factory method that takes [<Term>ChronoUnit</Term>](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/temporal/ChronoUnit.html).
    ```java
...
1. [<Term>Instant</Term>](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/time/Instant.html) specifies moment in **GMT time zone**.
    ```java
...
1. `private` - same class
    ...
    Even if 2 classes are defined in the same java file, they <Warn>cannot</Warn> access private fields or methods of each other.
    :::
...
1. `protected` - same package or subclass.
    - There are 2 ways to access protected members.
        ...
        public class Parent {
            ...
            public static void main(String... args) {
                Child child = new Child(); // Only for same package and subclass
                Peer peer = new Peer();
                peer.createParent(); // Only for same package and subclass
            }
        }
        ```
    - Please refer to book examples.
1. `public` - Anyone
#### <Term>Optional Specifier</Term>
...
1. Only `final` modifier can be applied to local variables.
1. For final primitives, once it is assigned, its value <Warn>cannot</Warn> be modified.
1. For final object, once it is assigned, its *reference* <Warn>cannot</Warn> be modified. Its content can however be modified.
1. <Term>Effectively final</Term> means the variable is not modified after assignment, even if it is not marked as final. Compiler can detect it.
...
1. Compiler does not apply default value to final instance or class variables.
1. If instance variables are marked final, they must be assigned a value when it is declared or when the object is instantiated (Either in constructor or initializer).
...
1. Use **static initializer** to initialize static final variables.
    ...
    ```
1. Instance Initializer / Constructor <Warn>cannot</Warn> initialize static final variables.
...
1. Static import <Warn>cannot</Warn> import class. To import class, use regular import instead.
1. You <Warn>cannot</Warn> static import 2 or more static members with the same name. In that case, refer to static member via their class name.
### Pass by value
...
1. When passing an object to a method, the method receives a copy of reference, and that copy of reference **points to the same object** as the caller.
...
1. <Term>Autoboxing</Term>: primitive -> wrapper class
1. <Term>Unboxing</Term>: wrapper class -> primitive
1. Java can implicitly cast a smaller primitive to a larger one. It can implicitly autobox a primitive to wrapper class. But it <Warn>cannot</Warn> do both at the same line.
    ```java
    float f = 1.0f;
    double d1 = f; // Implicitly cast a smaller primitve to larger one
    Double d2 = d1; // Implicitly autobox a primitive to wrapper class
    // This will error
...
1. Unboxing a null value will throw NullPointerException.
    ...
    ```
### Overloading
1. Overloading methods have the same method name but different parameter lists. 
1. Overloading methods must have different method signature. 
1. You <Warn>cannot</Warn> declare duplicate methods (same method signature) in the same class.
...
1. The same applies to autoboxing when both primitive and wrapper version are present as overloading methods. Java picks the most specific version.
    ...
    ```
1. Array <Warn>cannot</Warn> perform autoboxing as below.
    ```java
...
1. Java treats varargs as if they were an array. Method signature will be the same as an array, and lead to duplicate method error.
    ...
    ```
1. Overloading order is listed in order of precedence below.
    - Exact match by type
...
1. Private members are <Warn>NEVER</Warn> available via inheritance.
1. You <Warn>cannot</Warn> `extends` a **final** class.
...
### Access modifier
1. A top level class is one not defined in another class, i.e. not <Term>Nested class</Term>.
1. You don't have to declare a top level class `public`.
    ```java
    // Highlight next line
    class Test {}
    ```
1. You <Warn>cannot</Warn> declare a top level class `private` or `protected`.
    ```java
    // This will error
    private class Test {}
    ```
...
1. To use the instance variable instead of the local variable, use the keyword `this`.
    ...
    Peter
    ```
1. `this` <Warn>cannot</Warn> be used in static method or static initializer.
    ```java
    public class Test {
        String name = "Peter";
    ...
    ```
1. The use of `this` is optional (if local variable with the same name is not available). When Java encounters a variable, it will check the class hierarchy.
    ```java
    public class Test {
        String name = "Peter";
        public void print(){
...
1. You can refer a parent variable or method with `super`.
    ...
    ```
1. Unlike `this`, `super` excludes any members found in the current class.
    ```java
    class Parent{}
    class Child extends Parent{
        String name = "Peter";
    ...
    ```
1. Since `this` already includes inherited members, `super` is only used when you have a naming conflict via inheritance, and that you need to use the member of parent specifically.
### Constructor
1. A constructor has the same name as the class.
1. A constructor has no return type.
1. Parameter type <Warn>cannot</Warn> be var.
1. A class can have multiple constructors with different signature. This is called <Term>Constructor overloading</Term>.
1. Constructor is used when creating a new object. This is called <Term>instantiation</Term>.
1. If you don't include any constructors in the class, Java will create a **do nothing, no-argument** <Term>Default constructor</Term>.
1. Compiler only inserts the default constructor when no constructor is defined.
    ...
    ```
    ```txt title='Result'
    java: constructor Test in class Test <Warn>cannot</Warn> be applied to given types;
        required: java.lang.String
        found:    no arguments
        reason: actual and formal argument lists differ in length
    ```
1. A private constructor <Warn>cannot</Warn> be called by other classes.
...
1. A constructor can call one another using this().
    ...
    ```
1. **this()** call must be the first statement in constructor.
    ```java
    public class Test
    {
        public Test(){
    ...
    ```
1. A constructor <Warn>cannot</Warn> use this() to call itself.
    ```java
    public class Test
    {
        public Test(){
...
1. Like `this()`, `super()` call must also be the first statement in constructor.
    ```java
    class Parent{
        public Parent(){
            System.out.println("I am Parent");
...
1. Java compiler automatically inserts a call to the no-argument constructor `super()` if you do not explicitly call `this()` or `super()` as the first line of a constructor.
    ...
    I am Child
    ```
1. Java compiler only knows how to add no-argument default constructor. It does <Warn>NOT</Warn> detect and add default constructor that have arguments. You must understand how the compiler add default constructor and `super()`.
1. In case Java compiler <Warn>cannot</Warn> implicitly add default constructor due to absence of no-argument constructor in the parent class, you must explicitly call the correct parent constructor in the child constructor. 
    ```java
    class Parent{
        public Parent( String name ){}
    }
    ...
    java: constructor Parent in class Parent <Warn>cannot</Warn> be applied to given types;
    required: java.lang.String
    reason: actual and formal argument lists differ in length
    ```
...
1. <Term>Class initialization</Term> in the following order:
...
1. Unlike `final` **local** variables, which are not required to have a value (be initialized) unless they are actually used, `final` **instance** variables must be assigned a value.    
1. <Term>Instance initialization</Term> in the following order:
...
1. You <Warn>cannot</Warn> override or hide `private` methods since they are not inherited.
1. Rules for <Term>Overriding</Term> an instance method:
    1. **Same method signature** as its parent method.   
    1. **At least as accessible as** its parent method. Can be more accessible. <Warn>Cannot</Warn> be more restrictive.
...
1. A static method <Warn>cannot</Warn> be overridden.
1. A static method can be hidden.
1. When both the parent and the child defines a static method with the same signature, the parent one will be hidden when accessing with child reference. This is called <Term>Method hiding</Term>.
...
1. **Method hiding** if both methods in the parent and child are static.
    ...
    ```
1. **Method overriding** if both methods in the parent and child are <Warn>NOT</Warn> static.
...
1. Compilation error occurs if one is static and the other one is not static.
    ...
    ```
1. Variable <Warn>cannot</Warn> be overridden. It can only be hidden.
...
1. Hiding a method or variable replaces the member only if a child reference type is used.
    ...
    ```
1. You <Warn>cannot</Warn> override / hide a final method.
    ```java
    class Parent{
        final void welcome(){
            System.out.println("Hello");
...
1. The above rule only applies to inherited method. Private methods are redeclared, not overridden or hidden. So no compilation error occurs below.
    ...
    ```
### Abstract Class
1. You can use keyword `abstract` to define an <Term>Abstract class</Term>.
1. An abstract class <Warn>cannot</Warn> be instantiated.
...
1. Only instance method can be abstract. Variables, static methods etc <Warn>cannot</Warn> be abstract.
...
1. A method or class <Warn>cannot</Warn> marked as both `final` and `abstract`.
1. A method <Warn>cannot</Warn> be marked as both `private` and `abstract`.
1. A method can be marked as both `private` and `final`.
1. A `static` method <Warn>cannot</Warn> be marked `abstract`.
### Immutable Object
1. Java is an OOP. States are stored in object.
1. States are, after all, stored in variables. They can eventually end up as an object. A stateful object.
...
1. Some people may think, as long as the instance variables are defined as private, setter is not defined, and that the constructor is defined private, outsiders will be unable to control the instantiation process or change its internal value.
1. The cruel fact is, as long as reference type is involved and that the outsiders hold a reference of internal variables of your target object in whatever way due to whatever reason, even though they <Warn>cannot</Warn> reassign the reference, they can change the content of that reference.
1. <Term>Immutable objects</Term> pattern is an object-oriented design pattern in which an object <Warn>cannot</Warn> be modified after it is created.
1. Below is the strategy to make an object immutable:
    ...
    1. <Term>Defensive copying</Term>
        - **on the way in**: the input parameters passed to constructor for instantiation of the target object will first make a copy inside the constructor. This way, even the outsider may hold the reference of parameter and change its content, the target object is not affected.
...
### Interface
1. An <Term>Interface</Term> is an abstract data type that declares a list of abstract methods.
1. An <Term>Implicit modifier</Term> is a modifier that the compiler automatically inserts into the code.
1. Interface variable is implicitly `public`, `static`, `final`.
1. Interface method without a body is implicitly `abstract`.
1. Interface method without the private modifier is implicitly `public`.
1. Interface is implicitly `abstract`.
...
1. An interface <Warn>cannot</Warn> be `final`, as it is implicitly abstract. Also there is no sense marking an interface as final as no class can implement it.
1. A class implements an interface with the keyword `implements`.
1. A class can implement multiple interfaces.
    ...
    ```
1. A class <Warn>cannot</Warn> extends an interface.
    ```java
    ...
    ```
1. An interface extends another interface with the keyword `extends`.
1. An interface can extend multiple interfaces.
    ...
    ```
1. An interface <Warn>cannot</Warn> extend a class.
1. An interface <Warn>cannot</Warn> implement an interface.
    ```java
    interface InterfaceA {}
    class ClassA {}
    // Highlight next line
...
1. Interface is simply a contract. It defines a set of rules and methods that a class implementing it must follow.
...
1. Java supports inheriting the same method from 2 interfaces that have compatible method declarations.
    ...
    ```
1. You can define an <Term>Abstract interface method</Term> with the keyword `abstract`.
1. An abstract interface method must <Warn>not</Warn> have method body.
1. Interface supports concrete instance and class methods with method body.
1. For **instance method**, interface supports <Term>default</Term> (Implicitly public) and <Term>private</Term> method.
1. For **class method**, interface supports <Term>static</Term> (Implicitly public) and <Term>private static</Term> method.
1. Interface does <Warn>not</Warn> support method with protected access. A class cannot extends an interface.
1. Interface does <Warn>not</Warn> support method with package access. For syntax and backward compatibility reasons.
    :::tip
    - A method without access modifier in an interface is implicitly **public**. 
    - A method without access modifier in a class is implicitly **package**.
    - If Java interface supports package method, it will break existing codes. Adding new access modifier for package will further confuse people.
    :::
1. You can define a <Term>Default interface method</Term> <Warn>only</Warn> in an interface, with the keyword `default`.
1. You <Warn>cannot</Warn> define a default method in a class.
1. You must provide the method body for a default method.
1. You can override a default interface method in a class that implements the interface.
1. A default method is **implicitly public**.
1. A default method <Warn>cannot</Warn> be marked **abstract**, as it provides the method body.
1. A default method <Warn>cannot</Warn> be marked **final**, as it allows the class that implement the interface to override it.
1. A default method <Warn>cannot</Warn> be marked **static**, as it associates with the instance rather than the class.
1. If a class implements 2 or more interfaces that contain a default method with the same method signature, the class must override that default method.
    - You can use `super` to choose the default method we would like to use in this scenario.
        ```java title="irushu.chapter7.Test1"
        package irushu.chapter7;
        interface A{
            // Highlight next line
        ...
        ```
1. One important use of default method is backward compatibility. 
    :::tip
    - Suppose a new method is added for an interface. 
    - Without default method, existing classes that implements this interface will need to modify and provide implementation. 
    - With default method, existing classes will simply use the default methods. No modification is needed.
    :::
1. You can define a <Term>Static interface method</Term> with the keyword `static`.
1. A static interface method is **implicitly public**.
1. A static interface method <Warn>cannot</Warn> be *abstract* or *final*.
1. A static interface method is <Warn>not</Warn> inherited. Even the class implments the interface, the class still need an explicit reference to the name of interface when accessing the method.
    ```java title="irushu.chapter7.Test2"
    package irushu.chapter7;
    interface A2{
        static void methodA(){
    ...
    public class Test2 implements A2{
        void execute(){
            // This will error
            methodA();
            // Highlight next line
    ...
    ```
1. A <Term>Private interface method</Term>, be it `private` or `private static`, can <Warn>only</Warn> be declared and used within the interface.
1. A private interface method **reduces code duplication**.
1. A private interface method provides <Term>Encapsulation</Term>. It does not expose the internal working of the interface when not required.
1. You define a private interface method with the keyword `private`.
1. A private interface method must have method body.
1. A **private static** interface method can be called by **any** methods within the interface.
1. A **private instance** interface method can <Warn>only</Warn> be called by <Warn>non-static</Warn> instance methods within the interface.
1. An **abstract method** can <Warn>only</Warn> be called by <Warn>non-static</Warn> instance methods within the interface.
1. Summary 1
...
### Enum
1. <Term>Enum</Term> is like a fixed set of constants.
1. You can declare a simple enum as below.
    ```java title="irushu.chapter7.Test3"
    package irushu.chapter7;
    // highlight-start
    enum TrafficLight {
        RED, YELLOW, GREEN;
    ...
    public class Test3{
        public static void main(String... args){
            System.out.println(TrafficLight.RED); // RED
            System.out.println(TrafficLight.YELLOW); // YELLOW
            System.out.println(TrafficLight.GREEN); // GREEN
        }
    }
    ```
1. Enum values are considered constants and used with snake case.
1. Enum values always come first.
1. Enum provides `values()`, `name()`, `ordinal()` and `valueOf` methods.
    ```java title="irushu.chapter7.Test4"
    package irushu.chapter7;
    enum Rainbow {
    ...
    public class Test4{
        public static void main(String... args){
            // highlight-start
            for(var color: Rainbow.values()){
                System.out.println(color.name());
                System.out.println(color.ordinal());
            }
            Rainbow violet = Rainbow.valueOf("VIOLET");
            // highlight-end
            try{
                Rainbow brown = Rainbow.valueOf("BROWN");
            }
            catch(Exception e){
                // This will error
                e.printStackTrace(); // java.lang.IllegalArgumentException: No enum constant irushu.chapter7.Rainbow.BROWN
            }
    ...
    5
    ```
1. The method `ordinal()` shows that each enum value actually has an int value assigned by the system.
1. You can use enum value as case value in switch statement.
    ```java title="irushu.chapter7.Test5"
    package irushu.chapter7;
    enum WeekDay{
        ...
        String getActivity(WeekDay weekDay){
            // highlight-start
            return switch(weekDay){
                case SATURDAY -> "Go to rest Zzzz on %s".formatted(weekDay.name());
                case SUNDAY -> "Go to play ~~~~ on %s".formatted(weekDay.name());
                default -> "Go to work !!!! on %s".formatted(weekDay.name());
            };
            // highlight-end
        }
    }
    public class Test5 {
        public static void main(String... args){
            System.out.println(WeekDay.WEDNESDAY.getActivity());
            System.out.println(WeekDay.SATURDAY.getActivity());
            System.out.println(WeekDay.SUNDAY.getActivity());
        }
    }
    ```
    ```txt title="Output"
    Go to work !!!! on WEDNESDAY
    Go to rest Zzzz on SATURDAY
    Go to play ~~~~ on SUNDAY
    ```
1. You can create complex enum with constructors, fields and methods.
    ```java title="irushu.chapter7.Test6"
    package irushu.chapter7;
    enum AppleProductType{
        MACBOOK("MacBook"), IMAC("iMac"), IPAD("iPad"), IPHONE("iPhone"),
        APPLE_WATCH("Apple Watch"), APPLE_PENCIL("Apple Pencil"), MAGIC_TRACKPAD("Magic Trackpad"), MAGIC_MOUSE("Magic Mouse") ;
        private String name;
        private AppleProductType(String name){
            this.name= name;
        }
    ...
    enum MyAppleDevice{
        IPHONE_14_PLUS(AppleProductType.IPHONE, "14 Plus", "Black"),
        IPAD_AIR_3(AppleProductType.IPAD, "Air Generation 3", "Space Grey"),
        MACBOOK_M2(AppleProductType.MACBOOK, "M2 Pro", "Silver"),
        APPLE_PENCIL_1(AppleProductType.APPLE_PENCIL, "1", "White"){
            public String getDescription(){
                return """
                    ================================================================
                    Apple Product Type : %s
                        Apple Model No : %s""".formatted(productType.getName(), modelNo);
            }
        };
        // highlight-start
        // Fields
        protected AppleProductType productType;
        protected String modelNo;
        protected String color;
        // Constructor
        private MyAppleDevice(AppleProductType productType, String modelNo, String color){
        ...
        public String getDescription(){
            return """
                    ...
                    Apple Product Type : %s
                        Apple Model No : %s
                                Color : %s""".formatted(productType.getName(), modelNo, color);
        }
        // highlight-end
    }
    public class Test6 {
        public static void main(String... args){
            for(var myAppleDevice: MyAppleDevice.values()){
                System.out.println(myAppleDevice.getDescription());
            }
        }
    }
    ```
    ```txt title="Output"
    ================================================================
    Apple Product Type : iPhone
        Apple Model No : 14 Plus
                Color : Black
    ================================================================
    Apple Product Type : iPad
        Apple Model No : Air Generation 3
                Color : Space Grey
    ================================================================
    Apple Product Type : MacBook
        Apple Model No : M2 Pro
                Color : Silver
    ================================================================
    Apple Product Type : Apple Pencil
        Apple Model No : 1
    ```
1. You can define abstract method for enum.
1. An enum can implement interface.
## 8. Lambdas And Functional Interfaces
1. <Term>Functional programming</Term> is a way of writing code more <Term>declaratively</Term>.
1. You focus more on expressions than loops.


### Lambdas
1. <Term>Lambda expression</Term> is a block of code that gets passed around.
1. A lambda expression is like an unnamed method existing inside an anonymous class.
1. A lambda expression is also known as <Term>Closure</Term>.
1. Look at below program.
    ```java title="irushu.chapter8.Test1"
    package irushu.chapter8;
    ...
    public class Test1 {
        ...
        // highlight-start
        public static void printStaffList(List<Staff> lStaff, Filter filter){
            for(Staff staff: lStaff){
                if(filter == null){
                    System.out.println(staff.getName());
                }
                else if(filter.test(staff)){
                    System.out.println(staff.getName());
                }
            }
        }
        ...
        public static void main(String... argsß) {
            var lStaff = new ArrayList<Staff>();
            lStaff.add(new Staff("Mary", true));
            lStaff.add(new Staff("Tom", false));

            System.out.println("=== Non Permanent Staff (Lamda) ===");
            // Highlight next line
            printStaffList(lStaff, s -> !s.isPerm());
            
            System.out.println("=== Permanent Staff (Method Reference) ===");
            // Highlight next line
            printStaffList(lStaff, Staff::isPerm);
        }
    }

    class Staff{

        public Staff(String name, boolean isPerm) {
            this.name = name;
            this.isPerm = isPerm;
        }
    ...
    // highlight-end
    ```
 1. The line below uses a concept called <Term>Deferred execution</Term>. The code is specified now but will run later.
    ```
    printStaffList(lStaff, s -> !s.isPerm());
    ```
1. Lambdas work with interfaces that have **exactly one** abstract method. 
1. Java relies on <Term>Context</Term> when figuring out what lambda expressions mean. Context refers to where and how the lambda is interpreted.
1. In our example, Java tries to map our lambda to the abstract method declaration in the Filter interface.
    ```
    boolean test(Staff s);      
    ```
1. Below Lambdas are the same. Developers prefers the first one as they can do as little typing as possible. And it looks more simple.
    ```
    s -> !s.isPerm()    
    ```
    ```
    (Staff s) -> { return !s.isPerm(); }
    ```
1. The parentheses around the lambda parameters can be omitted only if there is a single parameter and its type is not explicitly stated.
1. Java allows you to omit a return statement and semicolon (;) when no braces are used. This special shortcut doesn’t work when you have two or more statements.
1. Below line of code cannot compile.
    ```java
    // This will error
    var staff = (Staff s) -> s.isPerm();
    ```
    Not enough context. Neither the lambda nor var have enough information to determine what type of functional interface should be used.

### Functional Interface
1. The `@FunctionalInterface` annotation tells the compiler that you intend for the code to be a functional interface.
1. If the interface does not follow the rules for a functional interface, the compiler will give you an error.
1. Just because you don’t see the annotation doesn’t mean it’s not a functional interface.
1. **Having exactly one abstract method** is what makes it a functional interface, not the annotation.
1. If a functional interface includes an abstract method with the same signature as a public method found in Object, those methods do not count toward the single abstract method test. For example, below methods won't count.
    ```
